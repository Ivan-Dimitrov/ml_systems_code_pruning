...
def main(args):
    ...
    for model in models:
        ...
        #middle_layers = [13, 14, 15, 16]
        #last_layers = [26,27,28,29]


        # # compression.full_prune(encoder, .02)
        # compression.mask_generation(encoder, .02)
        # print (encoder.convolutions[0].weight)
        # print (encoder.convolutions[0].weight_mask)
        compression.full_prune(encoder, .05)
        compression.full_prune(encoder, .05)
    ...
    if args.score_reference:
        print ("if score_reference")
        translator = SequenceScorer(models, task.target_dictionary)
    else:   
        print ("else score reference")
